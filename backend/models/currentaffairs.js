...
currentAffairs.getFileForSubscribedUser = async(_id,type,fileType)=> {
  const get = await db.connectDb("currentaffairs", currentAffairsSchema);
  let insData = await get.findOne({_id:_id,type:type,fileType:fileType});
        if (insData) {
            return insData;
        } else {
            return false
...
module.exports = currentAffairs,currentAffairsSchema
