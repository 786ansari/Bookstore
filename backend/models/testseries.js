...
testSeriesModel.getFileForSubscribedUser = async(_id,fileType)=> {
  const get =  await db.connectDb("testseries", testSeriesSchema);
  let insData = await get.findOne({_id:_id,fileType:fileType});
        if (insData) {
            return insData;
        } else {
            return false
...
module.exports = testSeriesModel,testSeriesSchema
